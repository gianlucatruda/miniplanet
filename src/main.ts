...
const planetMaterial = new THREE.MeshPhongMaterial({
  color: 0x228b22,
  shininess: 10,
}); // a natural green with some specular shine
...
function applyBurns(craft: Craft, deltaTime: number) {
  ...
  if (keysPressed["KeyA"] && craft.thrusterFuel > 0) {
    ...
    craft.thrusterFuel = Math.max(
      0,
      craft.thrusterFuel - rotationIncrement * 1000,
    );
  }
...
function animate() {
  ...
  craftRegistry.forEach((craft) => {
    ...
    const r =
      (craft.orbitRadius * (1 - craft.e * craft.e)) /
      (1 + craft.e * Math.cos(craft.angle));
    ...
    const r2 =
      (craft.orbitRadius * (1 - craft.e * craft.e)) /
      (1 + craft.e * Math.cos(craft.angle + deltaAngle));
    ...
    craft.mesh.lookAt(
      new THREE.Vector3().addVectors(craft.mesh.position, tangent),
    );
  });
    ...
    // Handle craft removal
      ...
      updatedCrafts.forEach((cd: any) => {
        ...
        if (cd.id !== clientId && craftRegistry.has(cd.id)) {
          ...
          craft.orbitLine = createOrbitLine(
            craft.orbitRadius,
            craft.e,
            craft.omega,
          );
